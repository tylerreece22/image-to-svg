...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def quantize(
        ...
        if palette:
            ...
            if self.mode != "RGB" and self.mode != "L":
                ...
                raise ValueError(msg)
            im = self.im.convert("P", dither, palette.image)
            new_im = self._new(im)
    ...
    def histogram(self, mask=None, extrema=None):
        ...
        if mask:
            mask.load()
            return self.im.histogram((0, 0), mask.image)
        if self.mode in ("I", "F"):
            if extrema is None:
    ...
    def entropy(self, mask=None, extrema=None):
        ...
        if mask:
            mask.load()
            return self.im.entropy((0, 0), mask.image)
        if self.mode in ("I", "F"):
            if extrema is None:
                extrema = self.getextrema()
    ...
    def __transformer(
        ...
        self.im.transform2(box, image.image, method, data, resample, fill)
...
def alpha_composite(im1, im2):
    ...
    im1.load()
    im2.load()
    return im1._new(core.alpha_composite(im1.image, im2.image))
...
def blend(im1, im2, alpha):
    ...
    im1.load()
    im2.load()
    return im1._new(core.blend(im1.image, im2.image, alpha))
...
def merge(mode, bands):
    ...
    for band in bands:
        band.load()
    return bands[0]._new(core.merge(mode, *[b.image for b in bands]))
